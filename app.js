...
if (require.main === module) {
	...
	initCommandLine(args, function(err) {
		...
		if (path.existsSync(args.dpath, 'config.yaml'))
			args.config = yaml.safeLoad(fs.readFileSync(args.dpath, 'config.yaml'));
		else
			args.config = require(path.join(args.dpath, 'config.json'));

		if (path.existsSync(args.dpath, 'settings.yaml'))
			args.config = yaml.safeLoad(fs.readFileSync(args.dpath, 'settings.yaml'));
		else
			args.settings = require(path.join(args.dpath, 'settings.json'));

		if (path.existsSync(args.dpath, 'custom.yaml'))
			args.config = yaml.safeLoad(fs.readFileSync(args.dpath, 'custom.yaml'));
		else
			args.custom = require(path.join(args.dpath, 'custom.json'));

		if (path.existsSync(args.dpath, 'users.yaml'))
			args.config = yaml.safeLoad(fs.readFileSync(args.dpath, 'users.yaml'));
		else
